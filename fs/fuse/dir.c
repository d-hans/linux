...
static int fuse_mknod(struct user_namespace *, struct inode *, struct dentry *,
		      umode_t, dev_t);
static int fuse_open_nonatomic(struct inode *dir, struct dentry *entry,
					...
					int *alloc_inode)
{
	...
	struct qstr name;
	struct fuse_forget_link *forget = NULL;
			    ...
			    umode_t mode)
{
	...
	err = -ESTALE;
	new = fuse_open_atomic_revalidate(fm->fc, entry, &outentry, &alloc_inode);
	if (IS_ERR(new))
		goto out_free_ff;

	/* Old dentry is valid and already linked to inode */
	if (entry == new)
		instantiate = false;
